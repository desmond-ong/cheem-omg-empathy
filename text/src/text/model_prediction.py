...
parser.add_argument("-m", "--model", help="Saved model's path", required=True)
parser.add_argument('-d', '--difference', help="trained on different?: 0: No (actuall value), 1: Yes", default=1, type=int)
parser.add_argument("--data", help="Path to the to-be-evaluated dataset, e.g., data/Testing/transcripts_with_scores/", required=True)
parser.add_argument("--output", help="Prediction output dir, e.g., data/Testing/model_predictions/", required=True)
