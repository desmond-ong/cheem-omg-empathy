...
parser.add_argument('-t', '--threshold', help="threshold for changes, < 0: no threshold", default=0.1, type=float)
...
model_pred_output = os.path.join(model_pred_output, "{}_seq_{}".format(model_id, threshold))
utils.mkdir(model_pred_output)
...
for fname, sequences in sampled_data.items():
    ...
    predicted_sequence = utils.get_predicted_sequence(predictions, tmp_info, len(gt_sequences[fname]), threshold=threshold)
