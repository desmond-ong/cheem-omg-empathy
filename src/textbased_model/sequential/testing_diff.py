...
parser.add_argument('-t', '--threshold', type=float, help="threshold for accepting changes; < 0: no threshold", default=-1.0)
...
print("lstm_dim: {}\tln1_dim: {}\tdropout: {}\tThreshold: {}\t".format(lstm_dim, ln1_dim, dropout, threshold))
...
model_pred_output = os.path.join(model_pred_output, "{}_diff_{}".format(model_id, threshold))
utils.mkdir(model_pred_output)
...
for fname, utters in data_tensor.items():
    ...
    for index, utter in utters.items():
        ...
        with torch.no_grad():
            pred_difference, hidden = model(X, hidden)
            last_output = pred_difference[-1].view(1, -1)

        difference = last_output[0].cpu().item()
        if threshold > 0:
            if abs(difference) < threshold:
                difference = 0
        # update sequence
        prev_predicted = prev_predicted + difference
        utils.add_value_to_sequence(prev_predicted, predicted_sequence, start_index, end_index)
